#!/usr/bin/env bash
#
# Deterministic command-line build/test for Tipsy.
#
...
#   scripts/xc.sh test    # run the TipsyTests unit suite
...
PROJECT="Tipsy.xcodeproj"
SCHEME="Tipsy"

# Pinned simulator. Repin via TIPSY_SIM; resolution is by name so it stays
# stable across machines even when the runtime gets a patch update.
DEVICE_NAME="${TIPSY_SIM:-iPhone 17}"
...
ensure_booted() {
  ...
  if [ -z "$udid" ]; then
    echo "error: simulator '$DEVICE_NAME' not found. Set TIPSY_SIM or create it." >&2
    exit 1
  fi
